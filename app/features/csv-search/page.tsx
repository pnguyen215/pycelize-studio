...
export default function CSVSearchPage() {
  ...
  const handleFileChange = async (file: File | null) => {
    ...
    if (file) {
      ...
      try {
        const info = await csvApi.getInfo(file);
  ...
  const handleSearch = async (searchRequest: Omit<SearchRequest, "file">) => {
    ...
    try {
      const response = await csvApi.search({
        ...searchRequest,
        file,
      });
      setResult(response);
  ...
  const handleFetchOperators = async (file: File) => {
    const response = await csvApi.suggestOperators(file);
    ...
    <div className="container mx-auto p-8 max-w-6xl">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground mt-2">
          Search and filter CSV data with multiple conditions and export results
        </p>
      </div>
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>
            Upload a CSV file to search and filter its data
          </CardDescription>
        </CardHeader>
