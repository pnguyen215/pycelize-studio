...
export const env = (
  typeof window === 'undefined'
    ? process.env
    : (window as typeof window & { env?: Record<string, string | undefined> }).env
) || {};
...
export const EEnv = Object.freeze({
   ...
   */
  NEXT_PUBLIC_PYCELIZE_API_URL: (env.NEXT_PUBLIC_PYCELIZE_API_URL as string) || "http://localhost:5050/api/v1",
   ...
   */
  NEXT_PUBLIC_DEBUGGING_REQUEST: (env.NEXT_PUBLIC_PYCELIZE_DEBUGGING === 'true') as boolean,
   ...
   */
  NEXT_PUBLIC_FIREBASE_CONFIG: JSON.parse((env.NEXT_PUBLIC_FIREBASE_CONFIG as string) || '{}') as Record<string, unknown>,
});
