...
import { defaultCacheManager, CacheManager } from '@/lib/services/cache-manager';
 ...
 */
function getCacheKey(config: InternalAxiosRequestConfig & CacheInterceptorConfig): string {
  if (config.cacheKey) {
 ...
 */
function shouldUseCache(config: InternalAxiosRequestConfig & CacheInterceptorConfig): boolean {
...
export async function cacheRequestInterceptor(
  ...
  if (config.invalidateCache) {
    if (typeof config.invalidateCache === 'string' || config.invalidateCache instanceof RegExp) {
...
export function cacheResponseInterceptor(
  ...
  const config = response.config as InternalAxiosRequestConfig & CacheInterceptorConfig & { _useCache?: boolean };
